...
public class AutoSpellingTextArea extends StyledTextArea<Boolean> {
	...
	private model.AutoCompleteDictionaryTrie ac;
	private model.DictionaryHS dic;
	private model.SpellingSuggest ss;
	...
	public AutoSpellingTextArea(model.AutoCompleteDictionaryTrie ac, model.SpellingSuggest ss, model.DictionaryHS dic) {
	...
	private void showSuggestions(String word, MouseEvent click) {
		...
		item.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				dic.addWord(word);
				((model.Dictionary)ac).addWord(word);
