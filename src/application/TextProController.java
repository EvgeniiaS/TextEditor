...
public class TextProController {
	...
	private void initialize() {
		...
		model.DictionaryHS dic = launch.getDictionary();
	...
	private void handleFleschIndex() {
		...
		if(!text.equals("")) {
			...
			model.Document doc = launch.getDocument(text);
	...
	private void handleMarkovText() {
		// get MTG object
		model.MarkovTextGenerator mtg = launch.getMTG();
		
		Task<model.MarkovTextGenerator> task = new Task<model.MarkovTextGenerator>() {
	        @Override
	        public model.MarkovTextGenerator call() {
	    ...
	    task.setOnSucceeded(e -> {
	    	loadStage.close();
	        model.MarkovTextGenerator result = task.getValue();
